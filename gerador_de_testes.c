...
void povoaImagem (GeradorDeTestes* g, Imagem3C* img);
int insereVeiculo (GeradorDeTestes* g, Imagem3C* img, int row_v, int col_v);
Imagem3C* copiaImagem (Imagem3C* img);
void suavizaImagem (Imagem3C* img);
double rand_normal (double mean, double stddev);
void adicionaRuido (Imagem3C* img);
...
GeradorDeTestes* criaGeradorDeTestes ()
{
	...
	g->img_veiculos = (Imagem3C**) malloc (sizeof (Imagem3C*) * N_VEICULOS);
	for (i = 0; i < N_VEICULOS; i++)
	{
		g->img_veiculos [i] = abreImagem3C (VEICULOS [i]);
		if (!g->img_veiculos [i])
			return (NULL);
	}

	/* Lê o fundo e a imagem dos caminhos. */
	g->background = abreImagem3C (BACKGROUND_FILE);
	g->path = abreImagem1C (PATHS_FILE);
...
void destroiGeradorDeTestes (GeradorDeTestes* g)
{
	...
	for (i = 0; i < N_VEICULOS; i++)
		destroiImagem3C (g->img_veiculos [i]);
	free (g->img_veiculos);
...
void povoaImagem (GeradorDeTestes* g, Imagem3C* img)
{
	...
	for (row = 0; row < g->path->altura; row++)
		for (col = 0; col < g->path->largura; col++)
			if (g->path->dados [row][col] && rand () % 1000 < chance)
			{
...
int insereVeiculo (GeradorDeTestes* g, Imagem3C* img, int row_v, int col_v)
{
	...
	if (col_v + img_veiculo->largura >= g->path->largura || !g->path->dados [row_v][col_v + img_veiculo->largura])
	...
	for (row = 0; row < img_veiculo->altura; row++)
		...
		{
			int x = col_v + col;
			int y = row_v - img_veiculo->altura/2 + row;

			if (x >= 0 && x < (int) img->largura && y >= 0 && y < (int) img->altura)
			{
				/* Magenta == translúcido. */
				if (img_veiculo->dados [0][row][col] == img_veiculo->dados [2][row][col] &&
					img_veiculo->dados [1][row][col] == 0)
				{
				...
				{
					img->dados [0][y][x] = img_veiculo->dados [pega_r][row][col];
					img->dados [1][y][x] = img_veiculo->dados [pega_g][row][col];
					img->dados [2][y][x] = img_veiculo->dados [pega_b][row][col];
				}
			}
		}
 ...
 *
 * Parâmetros: Imagem3C* img: imagem a copiar.
 *
...
Imagem3C* copiaImagem (Imagem3C* img)
{
	...
	copia = criaImagem3C (img->largura, img->altura);
 ...
 *
 * Parâmetros: Imagem3C* img: imagem a suavizar.
 *
 ...
 *
 * Parâmetros: Imagem3C* img: imagem à qual adicionaremos ruído.
 *
 * Valor de retorno: nenhum. */
